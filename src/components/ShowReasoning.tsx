/**
 * ShowReasoning Component
 * Displays AI agent reasoning and thought logs
 */
...
export interface ShowReasoningProps {
  section: string;
  title: string;
  ...
  onCopyNotification?: (position: { x: number; y: number }) => void;
  onAskAgent?: (payload: string, section: string) => void;
  onAuditTrace?: (entry: ThoughtLogEntry, section: string, displayName?: string) => void;
  editingThoughtId?: string | null;
  editPrompt?: string;
  setEditPrompt?: (prompt: string) => void;
  editToolInputs?: Record<string, any>;
  setEditToolInputs?: (inputs: Record<string, any>) => void;
  beginEdit?: (entry: ThoughtLogEntry, section: string) => void;
  cancelEdit?: () => void;
  saveEdit?: (entry: ThoughtLogEntry, section: string) => void;
  rerunThinking?: (entry: ThoughtLogEntry, section: string) => void;
  thinkingEntry?: ThinkingEntry | null;
  thinkingText?: string;
}

const ShowReasoning: React.FC<ShowReasoningProps> = ({
  section,
  title,
  logs = [],
  currentSection,
  ...
  const triggerCopyNotification = (event: React.MouseEvent) => {
    if (onCopyNotification) {
      onCopyNotification({ x: event.clientX, y: event.clientY });
    }
  };

  const formatEntries = (formatter: (entry: ThoughtLogEntry, index: number) => string) =>
    safeLogs.map(formatter).join('\n\n\n');

  const handleClipboardCopy = (event: React.MouseEvent, formatter: () => string) => {
  ...
  const handleCopyPrompts = (event: React.MouseEvent) => {
  ...
  const handleCopyOutputs = (event: React.MouseEvent) => {
    handleClipboardCopy(event, () =>
      formatEntries(
        (entry, index) =>
          `=== Entry ${index + 1}: ${entry.title} ===\nTime: ${entry.time}\n\nOUTPUT:\n${entry.output}`
  ...
  const handleCopyToolInputs = (event: React.MouseEvent) => {
    ...
    handleClipboardCopy(event, () =>
      toolEntries
        .map(
          (entry, index) =>
            `=== Entry ${index + 1}: ${entry.title} ===\nTime: ${entry.time}\nTool: ${entry.tool?.name || 'Unknown'}\n\nTOOL INPUTS:\n${JSON.stringify(entry.tool?.inputs || {}, null, 2)}`
        )
        .join('\n\n\n')
    );
  };

  const handleCopyAll = (event: React.MouseEvent) => {
    handleClipboardCopy(event, () =>
      formatEntries((entry, index) => {
    ...
    <div className="thinking-popover-centered" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="thinking-popover-content">
        ...
        <div className="thinking-actions">
          <button className="analysis-action-button secondary" onClick={handleCopyPrompts}>
            Copy Prompts
          </button>
          <button className="analysis-action-button secondary" onClick={handleCopyOutputs}>
            Copy Outputs
          </button>
          {hasToolInputs && (
            <button className="analysis-action-button secondary" onClick={handleCopyToolInputs}>
              Copy Tool Inputs
            </button>
          )}
          <button className="analysis-action-button secondary" onClick={handleCopyAll}>
            Copy All
          </button>
            ...
            {safeLogs.length === 0 ? (
              ...
              safeLogs.map((entry) => {
                ...
                if (!subSectionName && baseTitle.includes(' - ')) {
                  ...
                  baseTitle = parts.shift() || baseTitle;
                  ...
                  <div key={entry.id} className="thinking-item">
                    <div className="thinking-item-header">
                      <div className="thinking-item-meta">
                        ...
                        <span className={`thinking-kind pill kind-${entry.kind}`}>
                          {entry.kind}
                        </span>
                      </div>
                        ...
                        {isEditing ? (
                          <>
                            <button
                              ...
                              onClick={() => saveEdit?.(entry, section)}
                            >
                              Save
                            </button>
                            <button className="tiny-button ghost" onClick={cancelEdit}>
                              Cancel
                            </button>
                          </>
                        ) : (
                          <>
                            <button
                              ...
                              onClick={() =>
                                onAuditTrace?.(entry, section, subSectionName || baseTitle)
                              }
                            >
                              Audit Trace
                            </button>
                            <button
                              className="tiny-button"
                              onClick={() => beginEdit?.(entry, section)}
                            >
                              Edit
                            </button>
                            <button
                              className="tiny-button"
                              onClick={() => rerunThinking?.(entry, section)}
                            >
                              Re-run
                            </button>
                          </>
                    ...
                    <div className="thinking-blocks">
                      ...
                      {entry.kind === 'tool' && (
                        <div className="thinking-block">
                          <div className="thinking-block-label">
                            Tool Inputs ({entry.tool?.name})
                          </div>
                          {isEditing ? (
                              ...
                              {Object.keys(editToolInputs || {}).map((key) => (
                                <label key={key} className="tool-input-row">
                                  ...
                                  <input
                                    ...
                                    onChange={(event) =>
                                      setEditToolInputs?.({ ...editToolInputs, [key]: event.target.value })
                                    }
                                  />
                                </label>
                              ))}
                            ...
                            <pre className="thinking-code">
                              <code>{JSON.stringify(entry.tool?.inputs || {}, null, 2)}</code>
                            </pre>
                          )}
                        </div>
                      )}
